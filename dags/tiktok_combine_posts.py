...
def get_tiktok_posts_combine_stats(**kwargs: Dict[str, Any]) -> None:
    parser_name = 'Tiktok Posts Combine'
    status = 'success'
    platform = 'tiktok'
    ...
    try:
        ...
        size = 100
        while True:
            ids = list(posts_collection.find({"platform": platform}, {"_id": 1}).skip(i * size).limit(size))
            i += 1
            if ids:
                for id_doc in ids:
...
tiktok_posts_combine_task = PythonOperator(
    task_id='get_tiktok_posts_combine_stats',
    python_callable=get_tiktok_posts_combine_stats,
    provide_context=True,
    dag=dag,
)

tiktok_posts_combine_task
