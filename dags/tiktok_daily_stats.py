...
def recalculate_tiktok_daily_stats(**kwargs: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Filter posts by platform 'tiktok'
        # posts = list(posts_collection.find({}))
        posts = list(posts_collection.find({"platform": "tiktok"}))
        ...
        for i in range(0, len(posts), batch_size):
            ...
            for p in batch:
                video_duration = None
                if 'video' in p and 'video' in p['video'] and 'duration' in p['video']['video']:
                    video_duration = p['video']['video']['duration']
                elif 'music' in p and 'duration' in p['music']:
                    video_duration = p['music']['duration']

                if video_duration is None:
                    print(f"Skipping video for post {p['_id']} due to missing 'video' information.")
                    continue

                video_desc = p['video']['desc'] if 'desc' in p['video'] else 'No description'
        ...
        # Delete only TikTok-related stats
        # daily_stats_collection.drop()
        daily_stats_collection.delete_many({"platform": "tiktok"})
        ...
        for day, day_videos in daily_views_map.items():
            ...
            for video in day_videos:
                ...
                data['followers'] += video['followers']
            if day_videos:
